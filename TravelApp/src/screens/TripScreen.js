...
import {
  ...
  Alert, ScrollView, Share, Modal, TextInput,
} from 'react-native';
...
export default function TripScreen({ navigation, route }) {
  ...
  const loadAll = useCallback(async () => {
    const [tvl, cats, pymnts] = await Promise.all([
      getTravelers(tripId), getCategories(tripId), getPayments(tripId)
    ]);
    setTravelers(tvl); setCategories(cats); setPayments(pymnts);
  ...
  const handleSavePayment = async () => {
    ...
    await insertPayment({ trip_id: tripId, from_traveler: payFrom, to_traveler: payTo, amount: amt, date: payDate, note: payNote.trim() });
    ...
    <TouchableOpacity style={styles.expCard}
      ...
      <View style={styles.expRight}>
        ...
        <View style={[styles.sharedBadge, { backgroundColor: item.is_shared ? COLORS.surfaceVariant : '#3a2a1a' }]}>
          <Text style={[styles.sharedText, { color: item.is_shared ? COLORS.textSecondary : COLORS.warning }]}>{item.is_shared ? 'wspólny' : 'wybrani'}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
            ...
            {payments.map(p => (
              <TouchableOpacity key={p.id} style={styles.payRow} onLongPress={() => handleDeletePayment(p)} activeOpacity={0.75}>
                <View style={styles.payLeft}>
                  <Text style={styles.payNames}>{p.from_name} → {p.to_name}</Text>
      ...
      <Modal visible={showPayModal} transparent animationType="slide" statusBarTranslucent>
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => setShowPayModal(false)}>
          <View style={styles.modalSheet} onStartShouldSetResponder={() => true}>
            ...
            {showPayDate && (
              <DateTimePicker value={new Date(payDate)} mode="date" display="default"
                onChange={(e, d) => { setShowPayDate(false); if (d) setPayDate(d.toISOString().split('T')[0]); }} />
            )}

            <Text style={styles.modalLabel}>Notatka (opcjonalnie)</Text>
            <TextInput
              style={styles.modalInput}
              placeholder="np. zwrot za hotel" placeholderTextColor={COLORS.textSecondary}
